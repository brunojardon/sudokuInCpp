...
int main(){
	...
	cout<<"\n 2) Solucionar   (Se generará un sudoku y se resolverá automáticamente) [No disponible]";
...
void resolver(){
	...
	do{
		...
		if(input>9 or ifi>8 or icol>8){
			system("echo Valor o valores incorrectos... && TIMEOUT -T 1");
		}else{
...
void proponer(){
	...
	switch(opc){
		...
		default:
			system("echo Opción invalida...");
			system("TIMEOUT -T 1"); //falta terminar <--------------------------
		break;
	}
}
